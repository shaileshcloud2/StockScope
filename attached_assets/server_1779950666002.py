"""
Positional Screener - Yahoo Finance (yfinance) x NSE 500
Python 3.11+ | Flask | Local-only | No API keys | No login

Run:     python server.py
Update:  python update_fundamentals.py
Open:    http://localhost:3000
"""

import os, sys, time, threading, logging, webbrowser
...
# ── Config ────────────────────────────────────────────────────────
SESSION_SECRET = os.getenv("SESSION_SECRET", "change_me_please_use_env")
...
SECTOR_INDEX_MAP = {
    ...
    "IT":                 ["^CNXIT"],
    "Auto":               ["^CNXAUTO"],
    # Sector-specific indices (per HTML rulebook)
    "Defence":            ["^CNXDEFENCE"],           # NIFTY INDIA DEFENCE
    "Chemicals":          ["^CEX"],                   # CHEMICALS INDEX
    "Healthcare":         ["^CNXPHARMA", "^NIFTY_HEALTHCARE"],  # NIFTY PHARMA + NIFTY HEALTHCARE
    "Pharma":             ["^CNXPHARMA"],             # alias
    # Thematic / related sectors
    "FMCG":               ["^CNXFMCG"],
    "Energy":             ["^CNXENERGY"],
    "Metals":             ["^CNXMETAL"],
    "Real Estate":        ["^CNXREALTY"],
    "Building Materials": ["^CNXREALTY"],             # proxy via Realty
    "Telecom":            ["^CNXMEDIA"],              # closest available
    "Media":              ["^CNXMEDIA"],
    "Logistics":          ["^CNXSERVICE"],
    "Services":           ["^CNXSERVICE"],
    "Aviation":           ["^CNXSERVICE"],            # proxy via Services
    "Infrastructure":     ["^CNXINFRA"],
    "Engineering":        ["^CNXINFRA"],              # proxy via Infra
    "Retail":             ["^CNXCONSUM"],             # NIFTY CONSUMPTION
    "Consumption":        ["^CNXCONSUM"],
    "New Age Tech":       ["^CNXIT"],                 # proxy via IT
}
...
CORS(app, origins=[f"http://localhost:{PORT}", f"http://127.0.0.1:{PORT}"], supports_credentials=True)
...
# ── yfinance data fetchers (individual ticker approach) ───────────
def fetch_stock_info(symbol, max_retries=2):
    """
    Fetch live quote info for a single NSE symbol using yf.Ticker.
    Returns dict with cmp, change, yr_high, yr_low, volume or None.
    """
    yf_sym = f"{symbol}.NS"
...
def fetch_historical(symbol, days=270, max_retries=2):
    """
    Fetch historical OHLCV using yf.Ticker.history.
...
def fetch_index_return(index_sym, period="1y"):
    """Fetch 1-year return for a Nifty sector index."""
    try:
...
def fetch_nifty_3m():
    """Fetch live 3-month return for Nifty 500 (^CRSLDX) via yfinance."""
    try:
...
def fetch_sector_tailwinds():
    """Fetch 1-year returns for Nifty sector indices."""
    index_perf = {}
    all_indices = set()
...
def run_scan():
    ...
    try:
        ...
        # Fetch quotes individually (more reliable than batch download)
        live_quotes = {}
        ...
        for idx, row in enumerate(symbols):
            ...
            try:
                ...
                if not hist_rows or len(hist_rows) < 50:
                    log.warning("Insufficient historical data for %s (%d rows), skipping", sym, len(hist_rows) if hist_rows else 0)
        ...
        scan["progress"] = 100
        scan["status"]   = f"Scan complete - {n_scored} scored, {len(skipped_symbols)} skipped, {len(failed_symbols)} failed, {len(no_data_symbols)} no data"
...
def auth_login():
    return jsonify({"ok": True, "name": "Guest", "msg": "No login required"})

@app.route("/auth/status")
def auth_status():
    return jsonify({"ok": True, "profile": {"name": "Guest"}, "msg": "No authentication required"})

@app.route("/auth/logout", methods=["POST"])
def auth_logout():
    return jsonify({"ok": True, "msg": "No session to clear"})
...
if __name__ == "__main__":
    print("\n" + "="*62)
    print("  Positional Screener - Yahoo Finance (yfinance) x NSE 500")
    print(f"  http://{HOST}:{PORT}")
    print(f"  Symbols loaded: {len(SYMBOLS)}")
    print(f"  Fundamentals:   {len(FUNDAMENTALS)} stocks cached")
    print(f"  Update funds:   python update_fundamentals.py")
    print(f"  Python:         {sys.version.split()[0]}")
    print(f"  Bind:           {HOST} (localhost-only)")
    print("  Stop:           Ctrl+C")
    print("="*62 + "\n")
